...
map.setPaintProperty("ny-zip-codes-dilq1b", "fill-opacity", ["case", ["boolean", ["feature-state", "hover"], false], 1, 0.5]);
map.on("mousemove", "nta", function (e) {
  if (e.features.length > 0) {
    activeNeighborhood = e.features[0].properties.NTAName

  }
})
map.on("mousemove", "ny-zip-codes-dilq1b", function (e) {
  if (e.features.length > 0) {
    if (hoveredZip) {
      var coordinates = [parseFloat(e.features[0].properties.INTPTLON10), parseFloat(e.features[0].properties.INTPTLAT10)];
      var zipcode = e.features[0].properties.ZCTA5CE10;
      if (zipCodeStats.hasOwnProperty(zipcode)) {
        //Do this
        var zipcodeRecord = zipCodeStats[zipcode];
        var evictions = zipcodeRecord.evictions;
        var population = zipcodeRecord.population;
        var borough = zipcodeRecord.borough;
        var income = zipcodeRecord.medianHouseholdIncome;
        var rtcText = zipCodesRTC.includes(zipcode) ? "Zipcode currently eligible for Right To Counsel" : "Zipcode not yet eligible for Right To Counsel"
        // var neighborhoodText = "Neighborhood: " + neighborhoods.find(x => x.zcta10 === zipcode).nameshort + "<br />";
        var neighborhoodText = "Neighborhood: " + activeNeighborhood + "<br />";
        // var neighborhoodsForZip = neighborhoods.find(x => x.zcta10 === zipcode);
        // var neighborhoodText = "
        // if (neighborhoodsForZip.length > 0) {
        //   neighborhoodText = "Neighborhoods: " + neighborhoodsForZip.filter(x => parseFloat(x.per_in_puma) > 0.1).map(x => x.nameshort + " (" + (x.per_in_puma * 100).toString() + "%)").join(" ") + "<br />";
        // }
        console.log(income);
        if (income == undefined){
          console.log("Hello");
          income = " not avaliable";
        }
        var description = "Zip Code: " + zipcode + "<br />"
        + "Borough: " + borough + "<br />"
        + neighborhoodText
        +"Eviction orders in 2017: " + evictions + "<br />"
        + "Population: " + population.toLocaleString() + "<br />"
        + "Median Household Income: $" + income.toLocaleString() + "<br />" +
        rtcText + "<br />"
...
map.on("mouseleave", "ny-zip-codes-dilq1b", function () {
